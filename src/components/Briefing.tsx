...
interface Props {
  ...
  line: string
  seconds: number
  /** Акцент экрана — им же красится анонимный автор. */
  accent: string
  /** Приписка под кнопкой — в смене напоминает правило про баги в проде. */
  note?: string
  /**
   * Автор скрыт. В смене вся вторая половина игры — понять, кто писал код,
   * и сделать это через `/git-blame`, а не прочитать в шапке. Поэтому там
   * ни портрета, ни имени, ни цвета: только `ai[bot]` и то, что он сказал.
   */
  anonymous?: boolean
  onStart: () => void
}
...
export function Briefing({
  ...
  // Цвет и имя выдают автора не хуже портрета, поэтому в анонимном режиме
  // они тоже подменяются: акцент экрана и безликое `ai`.
  ...
  return (
    <div className="screen-in mx-auto flex max-w-[900px] flex-col gap-4 px-[18px] pt-6">
      <div className="overflow-hidden rounded-2xl border border-[#26262c] bg-[#111116]">
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 border-b border-[#1f1f26] bg-[#0e0e12] px-5 py-3">
          ...
          <span className="font-mono text-[11px] text-[#6b6b77]">
            <span className="text-[#9a9aa4]">{name}</span>
        ...
        <div className="flex flex-wrap items-stretch gap-5 border-b border-[#1f1f26] p-5">
          <AgentAvatar
            ...
            color={color}
            size={176}
            style={{ boxShadow: `inset 0 1px 0 rgba(255,255,255,.08), 0 6px 0 #0b0b0e, 0 0 34px ${color}26` }}
          />
          ...
          <div className="flex min-w-[220px] flex-[1_1_260px] flex-col justify-center">
            ...
            <div className="mt-3 flex flex-wrap gap-[7px]">
              {pr.labels.map((label) => {
                const own = LABEL_COLOR[label] ?? color
                return (
                  <span
                    key={label}
                    className="rounded-full px-2.5 py-0.5 font-mono text-[11px]"
                    style={{ border: `1px solid ${own}55`, background: `${own}14`, color: own }}
                  >
                    {label}
                  </span>
                )
        ...
        <div className="flex flex-col gap-4 px-5 py-[18px]">
          ...
          <div
            ...
            style={{
              ...
              background: `linear-gradient(180deg, ${color}12, #101014 70%)`,
            }}
          >
      ...
      {/* В смене таймера нет — обещать секунды было бы враньём. */}
      <Button accent={color} onClick={onStart} autoFocus>
        {seconds > 0 ? `Открыть диф · ${seconds} секунд` : 'Открыть диф'}
      </Button>
