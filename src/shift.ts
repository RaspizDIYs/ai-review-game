 ...
 * ограниченный ресурс: каждая закрывает одну тихую мину наверняка.
 *
 * 05.08 снижено с трёх до двух. Вместе с шестью запросами терминала выходило
 * девять действий за смену, которые ничего не стоят по ходам, — игрок мог
 * прожить смену, почти не принимая решений.
 */
export const CLEANUPS = 2
...
/**
 * Платных запросов к терминалу на смену.
 *
 ...
 *
 * 05.08 снижено с шести до четырёх — по той же причине, что и уборки.
 * Заодно `/git-blame` перестал их тратить: история про автора не сужает
 * круг строк, платить за неё было не за что.
 */
export const PROBES = 4
...
/** Уборка сработала или разгребать было нечего — от этого зависит текст экрана. */
export interface Cleanup extends Turn {
  /** false — заряд не потрачен: тихих мин в проде нет. */
  done: boolean
}
 ...
 * а показывала минус. Двух зарядов на смену достаточно, чтобы уборка не стала
 * бесплатной, а ход для этого не нужен.
 *
 * **Пустая уборка заряд не тратит.** Раньше тратила: если тихих мин не было
 * (прод чист или всё, что в нём есть, уже рвануло — упавшее уборкой
 * не лечится), заряд списывался, здоровье прибавлялось, и из трёх уборок
 * до дела доходили две. Игрок платил за услугу, которую ему не оказали.
 * Да, отказ говорит «тихих мин нет» — но это честнее, чем молча забрать заряд.
 *
 * Скорость уборка не трогает: команда не замечает, что кто-то починил то,
 * что ещё не сломалось.
 */
export function cleanup(shift: Shift): Cleanup {
  // Заряды кончились — ничего не происходит.
  if (shift.cleanups <= 0) return { shift, fired: [], done: false }

  const target = weakest(shift.defects)
  if (!target) return { shift, fired: [], done: false }
  ...
  return {
    shift: {
      ...shift,
      prod,
      defects: without(shift.defects, target),
