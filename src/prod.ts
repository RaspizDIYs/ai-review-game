...
/**
  * Сколько здоровья снимает падение прода по весу дефекта.
  *
  * 04.08 подняты примерно в полтора раза. Смена стала короче (12 ходов вместо
  * 14), и появился терминал: слежка снимает треть цены пропуска, подсказки
  * поднимают долю находок. При старых числах средний ревьюер доезжал до конца
  * со здоровьем 70 — то есть петля «пропустил → прилетело» переставала
  * чувствоваться вовсе. Ориентир прежний: около сорока.
  */
const BLAST = [9, 15, 23, 30, 38]
 ...
 * у хорошего. Такой обрыв — не сложность, а лотерея. Скидка оставляет
 * несколько ходов на то, чтобы всё-таки найти виноватого.
 *
 * 04.08 поднято с половины до 0.7. С терминалом игнорировать аварии стало
 * слишком дёшево: профиль «не чинит вовсе» сгорал в 37% смен вместо 93%,
 * и диагностика переставала быть отдельным навыком с ценой.
 */
const REPEAT = 0.7
...
/**
  ...
  * 04.08 поднято с четырёх: слепая починка стала дешевле после того,
  * как терминал начал сужать круг подозреваемых.
  */
const REPAIR_MISS = 6
const REPAIR_BROKE = 8

/**
 * Итог починки. Удачная возвращает треть того, что мина успела снять,
 * — но не больше: сломанное в проде не отменяется задним числом.
 *
 * 04.08 было «половину». С выросшим BLAST половина возвращала столько,
 * что удачная диагностика почти отменяла аварию, и поднимать цену падений
 * становилось бессмысленно: игрок получал её обратно тем же ходом.
 */
export function afterRepair(
  ...
  if (result === 'cured') {
    const back = defect ? blastOf(defect) / 3 : CLEANUP_HEAL
